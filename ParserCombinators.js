...
function main(){
	var p = new SimpleExpressionGrammar().start.compress();
	...
	console.log("score="+(runs*1000.0/duration));
}
...
SequencingParser.prototype.then = function(p){
	var l = new Array();
	for(var i=0;i<this.subparsers.length;i++) l[i] = this.subparsers[i];
...
StarParser.prototype.parseWithContext = function(ctxt){
		...
		for(;;){
			var pos = ctxt.getPosition();
			try {
				l.push( this.p.parseWithContext(ctxt) );
			} catch(e) {
				ctxt.setPosition(pos);
				return l;
			}
...
WrappingParser.prototype.parseWithContext = function(ctxt){
		return this.t( this.p.parseWithContext(ctxt) );
}
...
ForwardReferenceParser.prototype.bind = function(p){
	if(this.forwardee!=null) throw "already bound";
		this.forwardee = p;
}
...
ForwardReferenceParser.prototype.parseWithContext = function(ctxt){
	throw "I should have been compressed away";
}
...
function SimpleExpressionGrammar(){
	...
	this.start.bind(this.exp.then(this.eoi()).wrap( function (o){ return o[0]; } ));
	...
	this.exp.bind(this.e1.then(this.plus.then(this.e1).star()).wrap(
		...
		this.parenExp.bind( this.lparen.then(this.exp).then(this.rparen).wrap( function(o){ return o[1]; } ));
		
		this.number.bind( this.digit.wrap(
